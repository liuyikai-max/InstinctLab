...
def main():
    ...
    # recursively find all retarget_poses_g1.h5 and save outputs alongside each source file
    src_tgt_pairs = []
    if os.path.isfile(args.src):
        if os.path.basename(args.src) != "retarget_poses_g1.h5":
            raise ValueError(f"Expected file name retarget_poses_g1.h5, but got: {args.src}")
        src_tgt_pairs.append(
            (
                args.src,
                os.path.join(os.path.dirname(args.src), "retarget_poses_g1_retargeted.npz"),
            )
        )
    else:
        for root, _, filenames in os.walk(args.src):
            for filename in filenames:
                if filename != "retarget_poses_g1.h5":
                    continue
                src_tgt_pairs.append(
                    (
                        os.path.join(root, filename),
                        os.path.join(root, "retarget_poses_g1_retargeted.npz"),
                    )
                )
